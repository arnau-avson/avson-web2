    ...
    <>
      ...
      <section style={{ background: 'var(--white)', padding: '80px 0 100px' }}>
        <div className="calc-container" ref={containerRef}>
          <div className="calc-progress" style={{ opacity: step === 4 ? 0.3 : 1 }}>
            <div className={progressClass(1)}><span className="calc-progress__label">Tu empresa</span></div>
            <div className={progressClass(2)}><span className="calc-progress__label">Sector público</span></div>
            <div className={progressClass(3)}><span className="calc-progress__label">Contratos</span></div>
          </div>
            ...
            <div className="calc-step" key="step1">
              ...
              <div className="pill-group">
                {[
                  ...
                  ['consultoria', 'Consultoría TIC'],
                  ['cloud', 'Servicios cloud'],
                  ['otro', 'Otro sector TIC']
                ].map(([val, label]) => (
          ...
          {step === 2 && (
            <div className="calc-step" key="step2">
              ...
              <div className="pill-group">
                {[
                  ['si', 'Sí, licitamos'],
                  ['intentado', 'Lo hemos intentado'],
                  ['quiero', 'Aún no, pero queremos']
                ].map(([val, label]) => (
              ...
              <div className="slider-wrap">
                ...
                <input type="range" min={5} max={80} value={sliderVal} step={5} onChange={(e) => setSliderVal(Number(e.target.value))} />
          ...
          {step === 4 && result && (
            <div className="calc-step" key="stepResult">
              ...
              <div style={{ background: 'var(--pearl)', padding: '28px 32px', marginBottom: '32px', borderLeft: '3px solid var(--gold)' }}>
                ...
                <div className="desglose-row" style={{ background: 'rgba(201,168,76,0.08)', margin: '0 -32px', padding: '12px 32px', borderBottom: 'none' }}>
                  <span style={{ fontWeight: 600, color: 'var(--navy)' }}>Oportunidad perdida anual estimada</span>
                  <strong style={{ fontSize: '16px', color: 'var(--gold)' }}>{result.formatted}</strong>
                </div>
              </div>
              <div style={{ background: 'var(--navy)', padding: '40px 40px 44px', textAlign: 'center' }}>
                ...
                <Link to="/contacto" style={{ display: 'inline-block', fontFamily: 'var(--font-sans)', fontSize: '12px', fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--navy)', background: 'var(--gold)', border: '1px solid var(--gold)', padding: '16px 40px', textDecoration: 'none' }}>Recuperar esos contratos →</Link>
              </div>
      ...
      {showGate && (
        <div className="gate-modal-overlay">
          <div className="gate-modal-box">
            <span className="label-tag">Tu análisis está listo</span>
            <h2>¿Dónde te enviamos el resultado?</h2>
            <p>Te enviamos el análisis completo con el desglose de contratos en riesgo y la propuesta para tu sector específico.</p>
            <form onSubmit={handleLeadSubmit}>
              ...
              <button type="submit">Ver mi análisis completo →</button>
              <p>Sin spam. Solo usamos tu email para enviarte el análisis.</p>
            </form>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        .gate-modal-overlay { display: flex; position: fixed; inset: 0; background: rgba(26,39,68,0.82); z-index: 9999; align-items: center; justify-content: center; padding: 20px; }
        .gate-modal-box { background: var(--white); max-width: 480px; width: 100%; padding: 48px 40px; position: relative; text-align: center; }
        .gate-modal-box h2 { font-family: var(--font-serif); font-size: 28px; font-weight: 400; color: var(--navy); margin-bottom: 12px; }
        .gate-modal-box p { font-family: var(--font-sans); font-size: 14px; color: var(--text-muted); margin-bottom: 24px; }
        .gate-modal-box input { width: 100%; padding: 14px 18px; margin-bottom: 16px; border: 1.5px solid #D8D8D8; border-radius: 8px; font-family: var(--font-sans); font-size: 14px; }
        .gate-modal-box button { background: var(--navy); color: var(--white); border: none; padding: 14px 24px; font-weight: 600; cursor: pointer; width: 100%; }
        @media (max-width: 600px) { .calc-container { padding: 0 20px; } .slider-val { font-size: 40px; } }
      `}</style>
    </>
  )
}