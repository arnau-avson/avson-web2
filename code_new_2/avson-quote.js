...
function avsonRenderQuote(q, el) {
  ...
  const rows = q.lines.map(function (l) {
    ...
    return '<tr><td style="padding:6px 0;color:#3D4A5C">' + l.label + tag + '</td>' +
           '<td style="padding:6px 0;text-align:right;color:#1f2023;font-weight:600">' + fmt(l.price) + '</td></tr>';
  }).join("");

  const expressRow = q.expressApplied
    ? '<tr><td style="padding:6px 0;color:#3D4A5C">Express (&lt;60 días) <span style="color:#e85838;font-weight:600">+' + (P.expressSurcharge * 100) + '%</span></td><td></td></tr>'
    : '';

  el.innerHTML =
    '<div style="background:#fff;border-radius:12px;padding:28px">' +
      '<p style="font-family:Inter,sans-serif;font-size:11px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;color:#e85838;margin:0 0 6px">Presupuesto orientativo · Empresa ' + q.size + '</p>' +
      ...
      '<div style="display:flex;justify-content:space-between;align-items:baseline;border-top:1px solid #E5E5E0;padding-top:12px;margin-top:4px">' +
        ...
        '<span style="font-family:\'Syne\',serif;font-size:34px;color:#1f2023;font-weight:600">' + fmt(q.base) + '<span style="color:#e85838">*</span></span>' +
      '</div>' +
      '<div style="background:#F7F7F5;border-radius:8px;padding:14px 16px;margin-top:16px">' +
        '<p style="font-family:Inter,sans-serif;font-size:13px;color:#1f2023;margin:0 0 6px;font-weight:600">Y si te decides ya:</p>' +
